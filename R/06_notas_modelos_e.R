...
mat <- with(trees, model.matrix(log(Volume) ~ log(Height) + log(Girth)))
mat
## tenemos tres variables en la regresion lineal
...
## Aplicacion interactiva y ayuda a las personas a interpretar sus modelos estadisticos
## Crea una tablita que nos ayuda a interpretar los coeficientes
...
## Le restamos la primera columna a la segunda para obtener ResponseResistant.treatmentpre

## Es necesario el cero para que no se tome batch1 como referencia
...
## RIN: entre más alto, menos RNAs rotos
...
## Explorariamos mas los datos para revisar que no haya otros problemas con las muestras y para explorar la relación entre nuestras variables.
...
## Componentes de regresion lienal a lo largo de todos los genes
eb_results <- eBayes(lmFit(vGene))
...
## El gen esta mas expresado en prenatal que en postnatal --> puntito de hasta arriba


## Otro modo de graficar
volcanoplot(eb_results, coef = 2, highlight = 3, names = de_results$gene_name)

## valor p más chiquito --> mejor analisis de expresion diferencial
